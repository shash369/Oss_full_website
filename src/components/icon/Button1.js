export default function Button1(){
    return (
        ...
        className="px-10 py-3 bg-gradient-to-r from-lime-500 to-lime-600 rounded-full shadow-xl group-hover:shadow-2xl group-hover:shadow-lime-600 shadow-lime-600 uppercase font-serif tracking-widest relative overflow-hidden group text-transparent cursor-pointer z-10 after:absolute after:rounded-full after:bg-lime-200 after:h-[85%] after:w-[95%] after:left-1/2 after:top-1/2 after:-translate-x-1/2 after:-translate-y-1/2 hover:saturate-[1.15] active:saturate-[1.4]"
      >
        Button
        <p className="absolute z-40 font-semibold bg-gradient-to-r from-lime-500 to-lime-700 bg-clip-text text-transparent top-1/2 left-1/2 -translate-x-1/2 group-hover:-translate-y-full h-full w-full transition-all duration-300 -translate-y-[30%] tracking-widest">
          WELCOME
        </p>
        <p className="absolute z-40 top-1/2 left-1/2 bg-gradient-to-r from-lime-700 to-lime-900 bg-clip-text text-transparent -translate-x-1/2 translate-y-full h-full w-full transition-all duration-300 group-hover:-translate-y-[40%] tracking-widest font-extrabold">
          in oss
        </p>
        <svg
        ...
        >
          <defs>
            <linearGradient id="sssurf-grad" y2="100%" x2="50%" y1="0%" x1="50%">
              <stop offset="0%" stopOpacity="1" stopColor="hsl(75, 100%, 50%)" /> {/* Light lime */}
              <stop offset="100%" stopOpacity="1" stopColor="hsl(75, 60%, 30%)" /> {/* Dark lime */}
            </linearGradient>
          </defs>
          <g transform="matrix(1,0,0,1,0,-91.0877685546875)" fill="url(#sssurf-grad)">
            {[0, 35, 70, 105, 140, 175, 210, 245].map((yOffset, index) => (
              <path
                key={index}
                opacity={(index + 1) * 0.05}
                transform={`matrix(1,0,0,1,0,${yOffset})`}
        ...
        <svg
          className="absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-[30%] group-hover:-translate-y-[33%] group-hover:scale-95 transition-all duration-500 z-40 fill-lime-500"
